...
function! mapcolor#printHexColor(color)
    if has_key(s:rgb_map, a:color)
        echo s:rgb_map[a:color]
    else
        echo 'No color for that key'
    endif
endfunction

function! mapcolor#getColor(color)
    if match(a:color, '#') >= 0
        call mapcolor#printVimColor(a:color)
    else
        call mapcolor#printHexColor(a:color)
    endif
endfunction
